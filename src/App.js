...
function App() {
  ...
  const handleTripSubmit = async (e) => {
    ...
    const formData = {
      ...
      groqApiKey: e.target.groqApiKey.value,
    };
    setLoading(true);
    setError(null);
    setHighlighted(null);
...
Additional Notes: ${formData.additionalNotes}
  ...
  try {
    ...
    if (!jsonMatch) {
      ...
      throw new Error('Invalid API response: No JSON block found');
    }
    ...
    try {
      ...
      console.error('JSON parsing error:', e, 'Extracted content:', jsonString);
